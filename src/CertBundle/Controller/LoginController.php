...
class LoginController extends Controller {
    ...
    public function checkAction(Request $request) {
        ...
        $Usuario = $this->usuarioAutenticadoLdap($username, $password);

        if ($Usuario) {
            $this->sesion->set('username', $username);
            $this->sesion->set('nombre', $Usuario['displayname'][0]);
            $this->sesion->set('email', $Usuario['mail'][0]);
            ...
            $status = "**ERROR EN ACCESO USUARIO: ".$username;
